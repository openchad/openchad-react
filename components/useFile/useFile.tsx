 ...
 * @param initialContent - Optional initial content if file doesn't exist
...
export function useFile(
    ...
    // Subscribe to file change events via WebSocket
    useEffect(() => {
        const file = files[fileKey];
        if (!file || !isStreamReady) return;
        // Subscribe and get the resolved file path
        let eventName = "";
        ...
        const setupSubscription = async () => {
            try {
                ...
                if (!mounted.current) {
                    // If unmounted during await, we might need to unsubscribe immediately if we successfully subscribed?
                    // But here we just got the path. The actual subscription logic is on the server side which we triggered.
                    // If we are the last subscriber, we should unsubscribe.
                    // However, the cleanup function below handles the unsubscription logic based on ref counts.
                    // We just need to make sure we don't add event listeners.
                    return;
                }
                if (result && result.path) {
                    ...
                    // Now set up the event listener with the correct path
                    if (mounted.current) {
                        window.addEventListener(eventName, handleFileChange);
                    }
                }
            } catch {
        ...
        if (file._subscriptionCount === 1) {
            ...
            // Already have the path, set up listener
        ...
        return () => {
            if (isStreamReady) {
                if (eventName) {
                    window.removeEventListener(eventName, handleFileChange);
                }
                const file = files[fileKey];
                if (file) {
                    // Decrement subscription count and unsubscribe on last subscriber
                    file._subscriptionCount--;
                    if (file._subscriptionCount === 0) {
                        // Last subscriber - tell Python to stop watching this file
